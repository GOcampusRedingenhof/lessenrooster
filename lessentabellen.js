/**
 * Dynamische Lessentabellen - Versie 2.2
 * Geïsoleerde, modulaire applicatie voor het tonen van lessentabellen bij GO Campus Redingenhof.
...
(function() {
  ...
  const LessentabellenApp = {
    ...
    config: {
      ...
      cssUrl: "https://cdn.jsdelivr.net/gh/GOcampusRedingenhof/lessenrooster@main/lessentabellen.css",
      cssUrlDev: null, // Voor ontwikkeling
     ...
     * @param {boolean} skipElementCreation - Als true, niet de DOM elementen aanmaken
     */
    ...
    /**
     * Laad de CSS dynamisch als die nog niet is geladen
     * @param {Function} callback - Functie om uit te voeren na het laden 
     */
    loadCSS: function(callback) {
      if (this.data.cssLoaded) {
        if (callback) callback();
        return;
      }
      
      // Controleer of de stylesheet al is ingeladen
      const existingStyles = document.querySelectorAll('link[href*="lessentabellen.css"]');
      if (existingStyles.length > 0) {
        this.data.cssLoaded = true;
        if (callback) callback();
        return;
      }
      
      // Bepaal de CSS URL (dev of prod)
      const cssUrl = this.config.cssUrlDev || this.config.cssUrl;
      
      // Voeg de stylesheet toe
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = `${cssUrl}?v=${this.version}`;
      link.id = 'lessentabellen-styles';
      ...
      link.onerror = () => {
        console.error('Fout bij het laden van lessentabellen CSS');
        // Probeer toch door te gaan
        if (callback) callback();
      };
    ...
    createRequiredElements: function() {
      ...
      if (!document.getElementById('les-container')) {
        console.log("Creating main container");
        const mainContainer = document.createElement('div');
        mainContainer.id = 'les-container';
        mainContainer.className = 'les-root';
        document.body.appendChild(mainContainer);
        
        // Voeg interne elementen toe
        mainContainer.innerHTML = `
          <div id="les-domains-container"></div>
          <div id="les-overlay"></div>
          <div class="les-wrapper" id="les-slidein" tabindex="-1">
            <button class="les-close-btn" aria-label="Sluiten">×</button>
            <h2 id="les-opleiding-titel">Lessentabel</h2>
            <p id="les-opleiding-beschrijving"></p>
            <div class="les-action-buttons">
              <a id="les-brochure-link" href="#" target="_blank">Brochure</a>
              <button id="les-print-button">Afdrukken</button>
            </div>
            <div id="les-lessentabel-container"></div>
            <div id="les-footnotes"></div>
            <img class="les-logo-print" src="https://images.squarespace-cdn.com/content/v1/670992d66064015802d7e5dc/5425e461-06b0-4530-9969-4068d5a5dfdc/Scherm%C2%ADafbeelding+2024-12-03+om+09.38.12.jpg?format=1500w" alt="Redingenhof logo" />
            <div class="les-datum">Afgedrukt op: <span id="les-datum-print"></span></div>
            <div class="les-quote">SAMEN VER!</div>
          </div>
        `;
      }
    },
    ...
    cacheElements: function() {
      const prefix = this.config.prefixClass.slice(0, -1); // Verwijder het streepje aan het einde
      
      this.elements.container = document.getElementById(`${prefix}-domains-container`);
      this.elements.slidein = document.getElementById(`${prefix}-slidein`);
      this.elements.overlay = document.getElementById(`${prefix}-overlay`);
      this.elements.tableContainer = document.getElementById(`${prefix}-lessentabel-container`);
    ...
    setupEventListeners: function() {
      ...
      document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && this.elements.slidein?.classList.contains(`${this.config.prefixClass}open`)) {
          this.closeSlidein();
        }
      });
    ...
    showLoading: function() {
      ...
      if (this.elements.container) {
        this.elements.container.innerHTML = `<div class="${this.config.prefixClass}loader-spinner"></div>`;
      }
    },
    ...
    hideLoading: function() {
      ...
      const spinner = this.elements.container?.querySelector(`.${this.config.prefixClass}loader-spinner`);
    ...
    handleError: function(message) {
      ...
      if (this.elements.container) {
        this.elements.container.innerHTML = `
          <div class="${this.config.prefixClass}error-message">
    ...
    /**
     * Cache data ophalen
     * @returns {Object|null} - Gecachte data of null
     */
     ...
     * @param {Array} data - Data om te cachen
     */
    ...
    /**
     * CSV parsen
     * @param {string} csvText - CSV tekst om te parsen
     * @returns {Array} - Geparsede CSV data
     */
     ...
     * @param {string} rawDomain - Ruwe domeinnaam
     * @returns {string} - Genormaliseerde domeinnaam
     */
     ...
     * @param {string} text - Tekst om te slugificeren
     * @returns {string} - Slugified tekst
     */
    ...
    buildGrid: function() {
      ...
      if (Object.keys(structuur).length === 0) {
        this.elements.container.innerHTML = `
          <div class="${this.config.prefixClass}not-found-message">
      ...
      for (const [normDomainKey, graden] of Object.entries(structuur)) {
        ...
        block.className = `${this.config.prefixClass}domain-block`;
        ...
        ["TWEEDE GRAAD", "DERDE GRAAD"].forEach(graadKey => {
          ...
          graadContainer.className = `${this.config.prefixClass}graad-container`;
          ...
          for (const [finaliteit, richtingen] of Object.entries(finaliteiten)) {
            ...
            finBlok.className = `${this.config.prefixClass}finaliteit-blok`;
     ...
     * @returns {Object} - Georganiseerde data
     */
    ...
    /**
     * Open slidein panel
     * @param {string} graad - Graad naam
     * @param {string} slug - URL slug
     * @param {string} normDomainKey - Genormaliseerde domeinnaam
     */
    openSlidein: function(graad, slug, normDomainKey) {
      ...
      const colors = this.config.domainColors[normDomainKey];
      if (colors) {
        this.elements.slidein.style.setProperty("--les-domain-base", colors.base);
        this.elements.slidein.style.setProperty("--les-domain-mid", colors.mid);
        this.elements.slidein.style.setProperty("--les-domain-light1", colors.light1);
        this.elements.slidein.style.setProperty("--les-domain-light2", colors.light1);
        this.elements.slidein.style.setProperty("--les-hover-row", colors.hover);
      ...
      this.elements.slidein.classList.add(`${this.config.prefixClass}open`);
      this.elements.overlay.classList.add(`${this.config.prefixClass}show`);
    ...
    closeSlidein: function() {
      ...
      this.elements.slidein.classList.remove(`${this.config.prefixClass}open`);
      this.elements.overlay.classList.remove(`${this.config.prefixClass}show`);
    ...
    /**
     * Bouw lessentabel
     * @param {Array} filteredData - Gefilterde data
     */
    buildLessonTable: function(filteredData) {
      ...
      if (showStageRow) {
        tableHTML += `
          <tr class="${this.config.prefixClass}stage-row">
            <td>Stage weken</td>
            ${klassen.map(klas => {
      ...
      if (footnotesElement) {
        ...
        if (uniqueFootnotes.length > 0) {
          footnotesElement.innerHTML = `
            <p class="${this.config.prefixClass}footnotes">${uniqueFootnotes.join(" &middot; ")}</p>
          `;
        } else {
          footnotesElement.innerHTML = "";
        }
